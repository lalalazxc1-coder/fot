...
import { MoreHorizontal, Edit2, History, UserX, X } from 'lucide-react';
...
export const EmployeeActionMenu: React.FC<EmployeeActionMenuProps> = ({ row, onHistory, onEdit, onDismiss, canManage, activeTab }) => {
        ...
        <div className="relative flex justify-end items-center h-full pr-2" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-10 top-1/2 -translate-y-1/2 flex items-center gap-1 bg-white p-1 rounded-xl shadow-lg border border-slate-200 z-50 animate-in fade-in slide-in-from-right-2 duration-200">
                    <button
                        onClick={(e) => { setIsOpen(false); onHistory(e, row); }}
                    ...
                    {canManage && activeTab === 'active' && (
                        <>
                            ...
                            <button
                                ...
                                className="flex items-center justify-center p-2 hover:bg-slate-50 text-slate-500 hover:text-emerald-600 rounded-lg transition-colors group relative"
                                title="Редактировать"
                            >
                            ...
                            <button
                                onClick={(e) => { e.stopPropagation(); setIsOpen(false); onDismiss(row.id); }}
