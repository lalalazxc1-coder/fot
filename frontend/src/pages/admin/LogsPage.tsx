        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between md:items-center w-full bg-white p-3 sm:p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    ...
                    <p className="text-slate-500 font-medium text-[13px] mt-0.5">Логи аудита и мониторинг сессий</p>
                </div>

                <div className="flex gap-1 bg-slate-50 p-1 rounded-xl w-full md:w-auto border border-slate-100 shrink-0">
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex flex-1 md:flex-none justify-center items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab.id
