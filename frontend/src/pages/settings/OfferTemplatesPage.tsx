    ...
    return (
        <div className="space-y-6 pb-20">
            <div className="flex flex-col md:flex-row gap-4 justify-between md:items-center w-full bg-white p-3 sm:p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-slate-800">Шаблоны офферов</h2>
                    <p className="text-slate-500 font-medium text-[13px] mt-0.5">Настройте варианты предложений для разных ролей или отделов</p>
                </div>
                <button
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="hidden md:grid grid-cols-[2fr_2fr_120px_120px_100px] gap-2 px-5 py-2.5 bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                ...
                {templates.length === 0 ? (
                    <div className="text-center py-12 text-slate-400 text-sm">
                        Нет созданных шаблонов. Создайте свой первый вариант.
                    </div>
                ) : (
                    <div className="divide-y divide-slate-100">
                        {templates.map((template) => (
                            <div key={template.id} className="grid grid-cols-1 md:grid-cols-[2fr_2fr_120px_120px_100px] gap-2 px-5 py-3 items-center hover:bg-slate-50/50 transition-colors group">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-slate-100 rounded-lg text-slate-400 group-hover:text-indigo-600 group-hover:bg-indigo-50 transition-colors">
                                        <Layout className="w-4 h-4" />
                                    </div>
                                    <span className="text-sm font-bold text-slate-900 truncate">{template.name}</span>
                                </div>
                                <div className="text-sm text-slate-600 font-medium truncate">{template.company_name || '---'}</div>
                                <div className="text-sm font-medium text-slate-900">{template.signatories?.length || 0} чел.</div>
                                <div className="text-sm font-medium text-slate-900">{template.benefits?.length || 0} пунктов</div>
                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => openEdit(template)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Настроить"><Edit3 className="w-4 h-4" /></button>
                                    <button onClick={() => { if (confirm('Удалить шаблон?')) deleteMutation.mutate(template.id) }} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Удалить"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            </div>
                        ))}
                    </div>
